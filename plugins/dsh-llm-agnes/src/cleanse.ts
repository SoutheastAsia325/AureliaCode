...
import { callsFromJson, segmentComplete, stripNoise, type Segment } from './segment.js';
...
/** 创建一份统计表。 */
export function createStats(): CleanseStats {
 ...
 * @returns 调用 id。
 */
function makeCallId(n: number): string {
...
function classify(text: string): string {
  ...
  if (hasFence && hasTag) return 'fence+tag';
  ...
  try {
    ...
    if (call !== undefined) return 'embedded-json';
  } catch {
    // 分类失败不影响清洗
  }
  return 'unknown';
 ...
 */
export async function* cleanseStream(
  ...
  /** 每个 index 的已知块类型。 */
  const blockTypeOf = new Map<number, string>();
  /**
  ...
  const streamedDeltas = new Set<number>();
  /** 本次流已生成的调用数。 */
  let callSeq = 0;

  if (stats) stats.streams += 1;

  /** 发出一个规范化的工具调用块（用新 index）。 */
  function* emitToolCall(call: { id?: string; name: string; arguments: string }): Generator<Chunk> {
    const idx = nextIndex;
    bump(idx);
  ...
  function* emitText(text: string): Generator<Chunk> {
  ...
  /** 按片段序列发出。 */
  function* emitSegments(segments: Segment[]): Generator<Chunk> {
    for (const seg of segments) {
      if (seg.kind === 'text') yield* emitText(seg.text);
      else for (const call of seg.calls) yield* emitToolCall(call);
    }
  }
  ...
  for await (const raw of source) {
    ...
    switch (chunk.type) {
      case 'block-start': {
        bump(chunk.index);
        if (typeof chunk.blockType === 'string') blockTypeOf.set(chunk.index, chunk.blockType);
        ...
        if (bt === 'text') {
          ...
          if (buffered !== undefined || !wasStreamed) {
            ...
            if (wasUndecided) {
              ...
              if (text.length > 0) yield { type: 'text-delta', index: chunk.index, text };
      ...
      case 'finish': {
        ...
        for (const [idx, text] of buffers) {
          ...
          if (hasCall) {
            ...
            if (cleaned.length > 0) yield { type: 'text-delta', index: idx, text: cleaned };
          }
        }
        buffers.clear();
