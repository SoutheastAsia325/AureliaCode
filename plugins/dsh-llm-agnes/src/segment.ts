...
export type Segment =
  ...
  | { kind: 'tool-calls'; calls: ExtractedToolCall[] };

/** 围栏开闭标记——剥离目标。 */
const FENCE_RE = /^```[a-zA-Z0-9_+-]*\s*$/;
const TAG_OPEN = ['<tool_call>', '<tool_calls>', '<function_call>'] as const;
const TAG_CLOSE = ['</tool_call>', '</tool_calls>', '</function_call>'] as const;
 ...
 */
const ENVELOPE_KEYS = ['tool_calls', 'toolCalls', 'tool_call', 'function_call', 'functionCall'] as const;
 ...
 * 数据对象几乎不可能同时长这样。
 */
const CALL_KEYS = ['arguments', 'input', 'parameters', 'args'] as const;
 ...
 * @returns 抽出的调用；空数组表示「这个 JSON 不是工具调用」。
 */
export function callsFromJsonRelaxed(value: unknown, relaxedBare: boolean): ExtractedToolCall[] {
  return callsFromJsonInner(value, relaxedBare);
}

export function callsFromJson(value: unknown): ExtractedToolCall[] {
 ...
 * @returns 抽出的调用。
 */
function callsFromJsonInner(value: unknown, relaxedBare: boolean): ExtractedToolCall[] {
  ...
  const argsSlotOf = (obj: Record<string, unknown>): ArgsSlot | undefined => {
    for (const key of CALL_KEYS) {
      if (key in obj) return { key, value: obj[key] };
    }
    return undefined;
  };
   ...
   */
  const accept = (obj: Record<string, unknown>, relaxed: boolean): boolean => {
    if (typeof obj['name'] !== 'string' || (obj['name'] as string).length === 0) return false;
    if (argsSlotOf(obj) === undefined) return false;
    if (relaxed || relaxedBare) return true;
    return typeof obj['id'] === 'string' && (obj['id'] as string).length > 0;
  };

  const emit = (obj: Record<string, unknown>, name: string, slot: ArgsSlot): void => {
    out.push({
  ...
  const visit = (node: unknown, relaxed: boolean): void => {
    if (node === null || typeof node !== 'object') return;
    if (Array.isArray(node)) {
      for (const item of node) visit(item, relaxed);
      return;
    }
    ...
    for (const key of ENVELOPE_KEYS) {
      if (key in obj) visit(obj[key], true);
    }
    ...
    if (fnNode !== null && typeof fnNode === 'object' && !Array.isArray(fnNode)) {
      const fnObj = fnNode as Record<string, unknown>;
      const slot = argsSlotOf(fnObj);
      if (slot !== undefined && typeof fnObj['name'] === 'string' && (fnObj['name'] as string).length > 0) {
        const corroborated =
          relaxed ||
          typeof obj['id'] === 'string' ||
          obj['type'] === 'function' ||
          'index' in obj;
        if (corroborated) emit(obj, fnObj['name'] as string, slot);
      }
    }

    // ③ 裸形态：{id, name, arguments}
    const slot = argsSlotOf(obj);
    if (slot !== undefined && accept(obj, relaxed)) emit(obj, obj['name'] as string, slot);
  };
...
export function scanJsonObjects(
  text: string,
  from = 0,
): { candidates: Candidate[]; pendingStart: number } {
...
export function candidateAt(
  text: string,
  start: number,
): { start: number; end: number; value: unknown } | undefined {
 ...
 */
export function segmentComplete(text: string): Segment[] {
   ...
   */
  const accepted: Array<{ cand: (typeof candidates)[number]; calls: ExtractedToolCall[] }> = [];
  ...
  for (const cand of candidates) {
    ...
    if (strict.length > 0) {
      if (prevAcceptedEnd >= 0) {
        ...
        if (stripNoise(gap).trim().length === 0) sawAdjacent = true;
      }
    ...
    if (probe.length > 0) prevAcceptedEnd = cand.end;
  }
  ...
  if (relaxedAllowed) {
    for (const entry of accepted) {
      ...
      if (relaxed.length > 0) entry.calls = relaxed;
    }
  }
  ...
  for (const entry of accepted) {
    ...
    if (calls.length === 0) continue; // 不是工具调用 → 留在文本里
    const between = stripNoise(text.slice(cursor, entry.cand.start));
    if (between.trim().length > 0) segments.push({ kind: 'text', text: between });
  ...
  if (tail.trim().length > 0) segments.push({ kind: 'text', text: tail });
  return segments;
}
